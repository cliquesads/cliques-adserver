 ...
 * @param click_url
 */
AdServerCLogger.prototype.click = function(request, response, click_url, cliquesModels){
    if (request.headers['user-agent'] != CLIQUESBOT_UA_STRING) {
        var click_meta = {
            type: 'CLICK',
            uuid: request.uuid,
        ...
        cliquesModels.getAllBidders(function(err, res){
            ...
            if (bidders.length > 0) {
                var request_options = {
                    // Not sure if this is the correct way to get bidder click event url
                    // "url": bidders[0].bidder.curl,
                    "url": 'http://35.196.104.183:18144',
                ...
                lib_request.post(request_options,function(err, response, body){
                    if (err) {
                        logger.info('Click event error: ' + err);
                        return;
                    } 
                });
            }
