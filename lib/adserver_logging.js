    ...
    if (request.headers['user-agent'] !== CLIQUESBOT_UA_STRING) {
        ...
        if (bidder){
            ...
            lib_request.post(request_options,function(err, response, body){
                if (err) {
                    this.error('Click event error: ' + err);
                    return;
                }
                this.info('Click event sent to bidder for clickId ' + click_meta.clickid + ', bidId '
                    + click_url.bidid +  '. RESPONSE_CODE: ' + response.statusCode);
            });
        } else {
            this.error("ERROR: No bidder found for clickId " + click_meta.clickid + ", bidId " + click_url.bidid
...
AdServerCLogger.prototype.s2s = function(eventType, url, method, retries, err, response){
    ...
    if (err) {
        ...
        conv_meta.responseMessage = err.message;
