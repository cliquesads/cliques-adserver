...
module.exports = function(db, logger, httpConfig){
    ...
    function renderCreativePayload(creative, secure, clickParams, callback){
        ...
        if (creative.type === 'native'){
            ...
            // augment with advertiser-taxonomy ID's in auctionData sub-obj
            // in payload, as these ID's are not available at auction time and need to
            // be populated on creative load.
            // For reference, `auctionData` is an object returned to client for external tracking purposes.
            // Tag constructors can specify which fields they want passed to click & impression trackers,
            // which include advertiser taxonomy identifiers. Goal was to contain this all in the exchange
            // but have to do some of it here because of order of operations.
            payload.adserverAuctionData = {
                "cid": creative.id,
