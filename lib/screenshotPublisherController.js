...
/**
 * Method to connect to redis database with the connection info given
 *
 * @param {Object} options - should either be empty, in which case the default redis host/port will be used, or contains specific redis connection information
 */
var connectToRedis = exports.connectToRedis = function(options) {
    options = options || {};
    ...
    redisConnection = redis.createClient(this.redis_port, this.redis_host, this.redis_options);
};
 ...
 * @param {Number} timeoutSecs - the time range in seconds
 * @param {Number} screenshotLimits - the counter value to trigger capture screen message publishing
 */
var storeIdPair = exports.storeIdPair = function(placementId, creativeGroupId, clickURL, timeoutSecs, screenshotLimits) {
        ...
        if (!reply) {
            ...
            if (elapsed > timeoutSecs * 1000) {
                ...
                if (valueInfo.count >= screenshotLimits) {
                    // reached screenshot limits, should publish a message to notify phantomjs to capture screen
                    redisConnection.set(keyForIdPair, initiateIdPairValue());
                    // TO-DO:::ycx
                } else {
