 ...
 */
var formPublishingMessageContent = function(creativeGroupId, websiteURL) {
    return `crgid=${creativeGroupId}&websiteURL=${websiteURL}`;
}
        ...
        if (!reply) {
            ...
            if (elapsed > timeoutSecs * 1000) {
                ...
                if (valueInfo.count >= screenshotLimits) {
                    ...
                    redisConnection.set(keyForIdPair, initiateIdPairValue());
                    pubsubService.publishers.createScreenshot(formPublishingMessageContent(creativeGroupId, websiteURL));
                } else {
